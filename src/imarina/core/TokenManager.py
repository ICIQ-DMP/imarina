...
class TokenManager:
    ...
    def get_token(self):
        ...
        if self.access_token is None or time.time() >= self.expires_at - 300:  # Refresca si falten <5 minuts
            self._refresh_token()
    ...
    def _refresh_token(self):
        ...
        try:
            ...
            raise RuntimeError(f"Error demanant el token d'accés: {e}\nResposta: {response.text}")
...
def _create_token_manager():
        ...
        if not tenant_id or not client_id or not client_secret:
            raise ValueError("Falten valors de TENANT_ID, CLIENT_ID o CLIENT_SECRET als secrets.")
